...
static void displayStuff(void)
{
	...
	{
		char str[300];
		sprintf(str,"Rendering: LUR (%s)",gAPI.getRasterizer()->getClassName());
		displayString(str,20,200);
		if(gDisplayStats)
		{
			sprintf(str,"Visited pixels: %ld",statistics::get(STATS_VISITED_PIXELS));
			displayString(str,20,180);
			sprintf(str,"Processed pixels: %ld",statistics::get(STATS_PROCESSED_PIXELS));
			displayString(str,20,160);
			sprintf(str,"Shaded pixels: %ld",statistics::get(STATS_SHADED_PIXELS));
			...
			int64 a = statistics::get(STATS_DEPTH_BUFFER_WRITE_BW)>>3L;
			sprintf(str,"Depth buffer BW: %I64d [R] + %I64d [W] = %I64d bytes = %5.3f Mbytes",statistics::get(STATS_DEPTH_BUFFER_READ_BW)>>3L,
				statistics::get(STATS_DEPTH_BUFFER_WRITE_BW)>>3L,tmp,float(double(tmp)/(1024.0*1024.0)));
			displayString(str,20,120);

			sprintf(str,"Depth clear BW: %I64d bytes = %5.3f Mbytes",statistics::get(STATS_DEPTH_CLEAR_BW)>>3L,
			...
			sprintf(str,"Color buffer BW: %I64d [R] + %I64d [W] = %I64d bytes = %5.3f Mbytes",statistics::get(STATS_COLOR_BUFFER_READ_BW)>>3L,
				statistics::get(STATS_COLOR_BUFFER_WRITE_BW)>>3L,tmp,float(double(tmp)/(1024.0*1024.0)));
			displayString(str,20,80);

			sprintf(str,"Color clear BW: %I64d bytes = %5.3f Mbytes",statistics::get(STATS_COLOR_CLEAR_BW)>>3L,
				float(double(statistics::get(STATS_COLOR_CLEAR_BW)>>3L)/(1024.0*1024.0)));
			displayString(str,20,60);

			// print texture stats
			sprintf(str,"Texture reads: %I64d = %5.3f Mbytes",statistics::get(STATS_TEXTURE_READ_BW)>>3L,
			...
			sprintf(str,"Total bandwidth (BW): %5.3f [R] + %5.3f [W] + %5.3f [C] = %5.3f Mbytes\n",
